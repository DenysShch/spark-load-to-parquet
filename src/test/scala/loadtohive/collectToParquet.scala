...
object collectToParquet {
  ...
  def worker(sc: SparkContext, sqlc:SQLContext, schema:StructType , typeSchema:Map[String, String], listOfPaths:List[String], outputPath:String, numPart:Int, fileCoalesce:Int):Unit = {
    ...
    .repartition(numPart).write.mode(SaveMode.Append).option("compression","gzip").parquet(outputPath)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setAppName(s"conv_to_par_$appName").set("spark.speculation","false")
    ...
    sqlc.sql("set spark.sql.parquet.compression.codec=gzip")
