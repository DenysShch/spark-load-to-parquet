...
COMMAND="jps -lm | grep -i TDR_AIU_PGFAIL.json"
...
then
      ...
      --name conv-to-parq[tdr_aiu_pgfail]
      --master yarn
      --deploy-mode cluster
      ...
      --files '$HOME/configs/TDR_AIU_PGFAIL.json'
      $HOME/lib/parquetLoader.jar TDR_AIU_PGFAIL.json
      "
      eval $sparkRun
fi
