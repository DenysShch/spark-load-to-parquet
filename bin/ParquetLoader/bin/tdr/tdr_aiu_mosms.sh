...
COMMAND="jps -lm | grep -i TDR_AIU_MOSMS.json"
...
then
      ...
      --name conv-to-parq[tdr_aiu_mosms]
      --master yarn
      --deploy-mode cluster
      ...
      --files '$HOME/configs/TDR_AIU_MOSMS.json'
      $HOME/lib/parquetLoader.jar TDR_AIU_MOSMS.json
      "
      eval $sparkRun
fi
