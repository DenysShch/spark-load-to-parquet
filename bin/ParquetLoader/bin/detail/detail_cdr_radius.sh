...
COMMAND="jps -lm | grep -i DETAIL_CDR_Radius.json"
...
then
      ...
      --name conv-to-parq[detail_cdr_radius]
      --master yarn
      --deploy-mode cluster
      ...
      --files '$HOME/configs/DETAIL_CDR_Radius.json'
      $HOME/lib/parquetLoader.jar DETAIL_CDR_Radius.json
      "
      eval $sparkRun
fi
