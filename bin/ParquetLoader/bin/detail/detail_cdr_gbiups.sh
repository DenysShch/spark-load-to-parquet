...
COMMAND="jps -lm | grep -i DETAIL_CDR_GbIuPS.json"
...
then
      ...
      --name conv-to-parq[detail_cdr_gbiups]
      --master yarn 
      --deploy-mode cluster 
      ...
      --files '$HOME/configs/DETAIL_CDR_GbIuPS.json'
      $HOME/lib/parquetLoader.jar DETAIL_CDR_GbIuPS.json
      " 
      eval $sparkRun
fi
