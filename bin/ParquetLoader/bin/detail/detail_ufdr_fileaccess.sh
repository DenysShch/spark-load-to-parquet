...
COMMAND="jps -lm | grep -i DETAIL_UFDR_FILEACCESS.json"
...
then
      ...
      --name conv-to-parq[detail_ufdr_fileaccess]
      --master yarn
      --deploy-mode cluster
      ...
      --files '$HOME/configs/DETAIL_UFDR_FILEACCESS.json'
      $HOME/lib/parquetLoader.jar DETAIL_UFDR_FILEACCESS.json
      "
      eval $sparkRun
fi
