...
COMMAND="jps -lm | grep -i DETAIL_UFDR_Streaming.json"
...
then
      ...
      --name conv-to-parq[detail_ufdr_streaming]
      --master yarn
      --deploy-mode cluster
      ...
      --files '$HOME/configs/DETAIL_UFDR_Streaming.json'
      $HOME/lib/parquetLoader.jar DETAIL_UFDR_Streaming.json
      "
      eval $sparkRun
fi
