...
COMMAND="jps -lm | grep -i DETAIL_CDR_S1MME.json"
...
then
      ...
      --name conv-to-parq[detail_cdr_s1mme]
      --master yarn
      --deploy-mode cluster
      ...
      --files '$HOME/configs/DETAIL_CDR_S1MME.json'
      $HOME/lib/parquetLoader.jar DETAIL_CDR_S1MME.json
      "
      eval $sparkRun
fi
