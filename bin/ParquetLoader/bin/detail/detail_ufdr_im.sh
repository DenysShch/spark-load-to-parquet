...
COMMAND="jps -lm | grep -i DETAIL_UFDR_IM.json"
...
then
      ...
      --name conv-to-parq[detail_ufdr_im]
      --master yarn
      --deploy-mode cluster
      ...
      --files '$HOME/configs/DETAIL_UFDR_IM.json'
      $HOME/lib/parquetLoader.jar DETAIL_UFDR_IM.json
      "
      eval $sparkRun
fi
