...
COMMAND="jps -lm | grep -i DETAIL_UFDR_EMAIL.json"
...
then
      ...
      --name conv-to-parq[detail_ufdr_email]
      --master yarn
      --deploy-mode cluster
      ...
      --files '$HOME/configs/DETAIL_UFDR_EMAIL.json'
      $HOME/lib/parquetLoader.jar DETAIL_UFDR_EMAIL.json
      "
      eval $sparkRun
fi
