...
COMMAND="jps -lm | grep -i DETAIL_UFDR_DNS.json"
...
then
      ...
      --name conv-to-parq[detail_ufdr_dns]
      --master yarn
      ...
      --files '$HOME/configs/DETAIL_UFDR_DNS.json'
      $HOME/lib/parquetLoader.jar DETAIL_UFDR_DNS.json
      "
      eval $sparkRun
fi
