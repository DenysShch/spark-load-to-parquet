...
COMMAND="jps -lm | grep -i DETAIL_UFDR_HTTP_Browsing"
...
then
      ...
      --name conv-to-parq[detail_ufdr_http_browsing]
      --master yarn
      ...
      --files '$HOME/configs/DETAIL_UFDR_HTTP_Browsing.json'
      $HOME/lib/parquetLoader.jar DETAIL_UFDR_HTTP_Browsing.json
      "
      eval $sparkRun
fi
