...
COMMAND="jps -lm | grep -i DETAIL_CDR_SGS.json"
...
then
      ...
      --name conv-to-parq[detail_cdr_sgs]
      --master yarn
      --deploy-mode cluster
      ...
      --files '$HOME/configs/DETAIL_CDR_SGS.json'
      $HOME/lib/parquetLoader.jar DETAIL_CDR_SGS.json
      "
      eval $sparkRun
fi
