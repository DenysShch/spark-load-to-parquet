...
COMMAND="jps -lm | grep -i DETAIL_CDR_CSFB_MOC.json"
...
then
      ...
      --name conv-to-parq[detail_cdr_csfb_moc]
      --master yarn
      --deploy-mode cluster
      ...
      --files '$HOME/configs/DETAIL_CDR_CSFB_MOC.json'
      $HOME/lib/parquetLoader.jar DETAIL_CDR_CSFB_MOC.json
      "
      eval $sparkRun
fi
