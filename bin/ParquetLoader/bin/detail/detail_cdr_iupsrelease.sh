...
COMMAND="jps -lm | grep -i DETAIL_CDR_IuPsRelease.json"
...
then
      ...
      --name conv-to-parq[detail_cdr_iupsrelease]
      --master yarn
      ...
      --files '$HOME/configs/DETAIL_CDR_IuPsRelease.json'
      $HOME/lib/parquetLoader.jar DETAIL_CDR_IuPsRelease.json
      "
      eval $sparkRun
fi
